...
read.xport <- function(file,
    ...
    file.header <-
        readBin(
            ...
            what = character(0),
            n = 1,
            size = nchar(xport.file.header)
        )
    file.header <-
        substr(file.header,
               start = 1,
               stop = nchar(xport.file.header))
    ...
    if (length(keep)) {
        ...
        whichds <-
            setdiff(names(dsinfo),
                    c(toupper(drop), '_CONTENTS_', '_contents_'))
    }
    
    scat("Reading the data file...")
    ds <- read.xport.inner(file, stringsAsFactors = FALSE)
    if (any(duplicated(names(dsinfo)))) {
        # only true if file contains has more than one data set
    ...
    if (!length(formats)) {
        fds <- sapply(dsinfo, function(x)
            all(
                c('FMTNAME', 'START', 'END', 'MIN', 'MAX', 'FUZZ')
                %in% x$name
            ))
        fds <- names(fds)[fds]
        if (length(fds) > 1) {
            warning(
                'transport file contains more than one PROC FORMAT CNTLOUT= dataset; using only the first'
            )
            fds <- fds[1]
        }
    }
    ...
    for (k in which.regular) {
        ...
        nam <-
            names.tolower(makeNames(names(w), allow = name.chars))
        names(w) <- nam
        dinfo <- dsinfo[[k]]
        ...
        ndinfo <-
            names.tolower(makeNames(dinfo$name, allow = name.chars))
        names(lab) <-
            names(fmt) <-
            names(formats) <- names(iformats) <- ndinfo
        if (length(w) > 0) {
            for (i in 1:length(w)) {
                changed <- FALSE
                x  <- w[[i]]
                ...
                names(fi) <- NULL
                if (fi != '' &&
                    length(finfo) && (fi %in% names(finfo))) {
                    f <- finfo[[fi]]
                    if (length(f)) {
                        ## may be NULL because had a range in format
                ...
                if (is.numeric(x)) {
                    ...
                    if ((is.logical(as.is) && !as.is) ||
                        (is.numeric(as.is) &&
                         length(unique(x)) < as.is * length(x))) {
                        x <- factor(x, exclude = '')
                        changed <- TRUE
                    }
                }
                ...
                if (nam[i] %in% names(formats)  &&
                    formats[nam[i]] > "") {
                    SASformat(x) <- formats[[nam[i]]]
                    changed <- TRUE
                }
                
                if (nam[i] %in% names(iformats) &&
                    iformats[nam[i]] > "") {
                    SASformat(x) <- formats[[nam[i]]]
                    changed <- TRUE
                }
